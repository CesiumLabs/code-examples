...
module.exports = (client) => {
	...
	class TextChannel extends Structures.get("TextChannel") {
		async createSlider(userID, embeds, emojiNext, emojiBack) {
			const button_back = new MessageButton().setStyle("red").setID("back");

			const button_next = new MessageButton().setStyle("green").setID("next");

			const button_back_disabled = new MessageButton().setStyle("gray").setID("back_disabled").setDisabled();

			const button_next_disabled = new MessageButton().setStyle("gray").setID("next_disabled").setDisabled();
			...
			const buttonsActive = new MessageActionRow().addComponents([button_back, button_next]);

			const buttonsDisabled = new MessageActionRow().addComponents([button_back_disabled, button_next_disabled]);

    ...
    <div>
        ...
        <div className="flex items-center">
          ...
          <p className="ml-4 leading-snug flex flex-col justify-between">
            <span className="text-white text-xl font-bold">
              Playing {isGitHub || "Nothing..."}
            </span>
            <span className="text-white">
              {activity?.details
                ?.split("", 35)
                .reduce(
                  (o, c) => (o.length === 34 ? `${o}${c}...` : `${o}${c}`),
                  ""
                ) || "Nothing..."}
            </span>
            <span className="text-white">
            ...
            <span className="text-white">
              ⏰ {timestamp.fromNow().split("ago")[0] || "0 minutes"} elapsed
            </span>
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-center">
        <h1 className="text-white font-semibold">
          Please input a Discord Snowflake id in the url. i.e{" "}
          <span className="focus:outline-none bg-none bg-gray-800 rounded-lg p-3 h-4 w-4">

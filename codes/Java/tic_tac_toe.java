...
public class tic_tac_toe {
	...
	private void PrintCurrentState() {
		System.out.println("| " + (hole1 > 0 ? this.getPlayerChar(hole1) : "hole1") + " | " + (hole2 > 0 ? this.getPlayerChar(hole2) : "hole2") + " | " + (hole3 > 0 ? this.getPlayerChar(hole3) : "hole3") + " |");
		System.out.println("-------------------------");
		System.out.println("| " + (hole4 > 0 ? this.getPlayerChar(hole4) : "hole4") + " | " + (hole5 > 0 ? this.getPlayerChar(hole5) : "hole5") + " | " + (hole6 > 0 ? this.getPlayerChar(hole6) : "hole6") + " |");
		System.out.println("-------------------------");
		System.out.println("| " + (hole7 > 0 ? this.getPlayerChar(hole7) : "hole7") + " | " + (hole8 > 0 ? this.getPlayerChar(hole8) : "hole8") + " | " + (hole9 > 0 ? this.getPlayerChar(hole9) : "hole9") + " |");
	}

	private boolean fillHole(int holeNumber, int player) {
		if (holeNumber == 1 && hole1 == 0) {
			hole1 = player;
			return true;
		} else if (holeNumber == 2 && hole2 == 0) {
			hole2 = player;
			return true;
		} else if (holeNumber == 3 && hole3 == 0) {
			hole3 = player;
			return true;
		} else if (holeNumber == 4 && hole4 == 0) {
			hole4 = player;
			return true;
		} else if (holeNumber == 5 && hole5 == 0) {
			hole5 = player;
			return true;
		} else if (holeNumber == 6 && hole6 == 0) {
			hole6 = player;
			return true;
		} else if (holeNumber == 7 && hole7 == 0) {
			hole7 = player;
			return true;
		} else if (holeNumber == 8 && hole8 == 0) {
			hole8 = player;
			return true;
		} else if (holeNumber == 9 && hole9 == 0) {
			hole9 = player;
			return true;
		}
		return false;

...
export default function Lanyard() {
  ...
  const isGitHub =
    activity?.name === "GitHub" ? (
      ...
      <span>
        <i className="fad fa-code text-blue-500"></i> Visual Studio Code
      </span>
    ) : (
      activity?.name
    );
    ...
    <div>
      <div className="rounded-md h-auto w-96 bg-gray-800 first:p-5 card">
        <div className="flex items-center">
          <Image
            src={activity ? `https://cdn.discordapp.com/app-assets/${activity?.application_id}/${activity?.assets?.large_image}.png` : "https://i.stack.imgur.com/y9DpT.jpg"}
      ...
      <div className="flex items-center justify-center">
        <h1 className="text-white font-semibold">
          Please input a Discord Snowflake id in the url. i.e <span className="focus:outline-none bg-none bg-gray-800 rounded-lg p-3 h-4 w-4">?id=567885938160697377</span>
        </h1>
      </div>
    </div>
  );

...
const searchCode = async(req: NextApiRequest, res : NextApiResponse) => {
    if (req.method === "POST") {
        ...
        const languages = fs.readdirSync(path.join(getConfig().serverRuntimeConfig.PROJECT_ROOT, 'public', 'codes'))
        ...
        languages.map((lang) => fs.readdirSync(path.join(getConfig().serverRuntimeConfig.PROJECT_ROOT, 'public', 'codes', lang), {withFileTypes:true}).map((name) => {
